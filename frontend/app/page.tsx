...
function TopBar({ active, onReport }: { active: PageId; onReport: () => void }) {
  const label = navItems.find((item) => item.id === active)?.label
  return <header className="topbar"><div className="mobile-menu"><Menu size={19} /></div><div className="breadcrumb"><span>Trace-Mark</span><ChevronRight size={14} /><strong>{label}</strong></div><div className="top-actions"><div className="security-chip"><span className="live-dot" />Secure session</div><button className="icon-button" aria-label="Notifications"><Bell size={17} /><span className="notification-dot" /></button><button className="avatar">RS</button><button className="report-button" onClick={onReport}><FileText size={15} /> Forensic report <ArrowUpRight size={14} /></button></div></header>
}
